...
public class UpgradeTable {
    ...
    private static final ItemStack item = ItemBuilder.builder()
            .material(UPGRADE_TABLE_MATERIAL)
            .name(ChatUtil.style("<word:light_purple>Upgrade Table"))
            .lore("<word:light_purple>Sometimes, you just need a little more power.")
            .glow(true)
            .build().make();
    ...
    public static final void setBlockAsUpgradeTable(ItemStack item) {
        item.getItemMeta().getPersistentDataContainer().set(identifierKey, PersistentDataType.BYTE, (byte) 1);
    }
